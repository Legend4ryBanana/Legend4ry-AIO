...
function Manager:__init()
	...
	elseif myHero.charName == "Katarina" then
		DelayAction(function() self:LoadKatarina() end, 1.05)
	elseif myHero.charName == "Draven" then
		DelayAction(function() self:LoadDraven() end, 1.05)
	end
end

function Manager:LoadKatarina()
	Katarina:Spells()
	Katarina:Menu()
	--
	--GetEnemyHeroes()
	Callback.Add("Tick", function() Katarina:Tick() end)
	Callback.Add("Draw", function() Katarina:Draw() end)
	if _G.SDK then
		_G.SDK.Orbwalker:OnPreAttack(function(...) Katarina:OnPreAttack(...) end)
		_G.SDK.Orbwalker:OnPostAttackTick(function(...) Katarina:OnPostAttackTick(...) end)
	end
end
...
function Katarina:Menu()
    self.Menu = MenuElement({type = MENU, id = "Katarina", name = "Katarina"})
    self.Menu:MenuElement({id = "ComboMode", name = "Combo", type = MENU})
    self.Menu.ComboMode:MenuElement({id = "UseQ", name = "Use Q in Combo", value = true})
    self.Menu.ComboMode:MenuElement({id = "UseW", name = "Use W in Combo", value = true})
    self.Menu:MenuElement({id = "KSMode", name = "KS", type = MENU})
...
function Katarina:CanUse(spell, mode)
    if mode == nil then
        mode = Mode()
    end
    --PrintChat(Mode())
...
function Katarina:Logic()
    ...
    if Mode() == "Combo" or Mode() == "Harass" and target then
        ...
        if self:CanUse(_E, Mode()) and ValidTarget(target, ERange) then
            self:UseW()
        end
    else
        WasInRange = false
...
function Katarina:OnPostAttackTick(args)
    attackedfirst = 1
    if target then
    end
end
...
function Katarina:OnPreAttack(args)

end

function Katarina:UseE(unit)
    Control.CastSpell(HK_E, unit)
end

function Katarina:UseQ(unit)
    Control.CastSpell(HK_Q, unit)
end
